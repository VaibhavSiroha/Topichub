...
# Load variables from a local .env file for development (no-op in production,
# where Vercel injects env vars directly and no .env file is deployed).
try:
...
#
# Uses a hosted database when DATABASE_URL is set (required on Vercel, since its
# filesystem is read-only and SQLite can't persist there). Falls back to a local
